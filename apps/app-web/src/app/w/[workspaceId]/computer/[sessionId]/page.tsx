...
export default function ComputerTakeoverPage(props: {
  ...
  const [captureStatus, setCaptureStatus] = useState<"idle" | "saved" | "failed">("idle");
  ...
  const onCaptured = useCallback(async () => {
    ...
    if (!target) return;
    setCapturing(true);
    const ok = await markComputerSessionCaptured(sessionId, target).catch(() => false);
    setCapturing(false);
    setCaptureStatus(ok ? "saved" : "failed");
  }, [sessionId, site]);
    ...
    <div className="flex h-full flex-col gap-3 p-4">
      ...
      <div className="flex flex-wrap items-end gap-2 rounded-lg border border-border p-3">
        ...
        {captureStatus !== "idle" ? (
          ...
          >
            {captureStatus === "saved" ? t.computer.captureSuccess : t.computer.captureFailed}
          </p>
        ) : null}
      </div>
    </div>
