...
/**
 * The session-vault port. The impl is closed (platform — envelope-encrypted
 * Postgres rows under workspace RLS); open-core boots without it and cloud
 * session reuse is simply unavailable. The browsing agent has NO tool over
 * this interface — only the orchestrator and Session Management routes call it.
 */
export interface SessionVault {
  get(params: { userId: string; workspaceId: string; site: string }): Promise<SessionBundle | null>
  put(params: { userId: string; workspaceId: string; site: string; bundle: SessionBundle }): Promise<void>
  /** Silent-death probe outcome: bundle no longer logs in. Kept for re-auth UX. */
  markDead(params: { userId: string; workspaceId: string; site: string }): Promise<void>
  touch(params: { userId: string; workspaceId: string; site: string }): Promise<void>
  list(params: { userId: string; workspaceId: string }): Promise<VaultSessionInfo[]>
  revoke(params: { userId: string; workspaceId: string; site: string }): Promise<void>
  /**
