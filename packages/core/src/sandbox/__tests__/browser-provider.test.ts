...
import {
  routeBrowserBackend,
  isAccountSensitiveUrl,
  DEFAULT_ACCOUNT_SENSITIVE_DOMAINS,
...
// ── Router (§4.15): account-sensitive → local, public → cloud ──

describe('[COMP:sandbox/browser-provider] Browser backend router', () => {
  it('routes an account-sensitive site (LinkedIn) to the local extension even when cloud is available', () => {
    expect(
      routeBrowserBackend({ url: 'https://www.linkedin.com/messaging/', cloudAvailable: true }),
    ).toBe('local')
  })

  it('routes a public task to the cloud backend when a sandbox provider is configured', () => {
    expect(
      routeBrowserBackend({ url: 'https://news.ycombinator.com/', cloudAvailable: true }),
    ).toBe('cloud')
  })

  it('falls back to local when no cloud backend is configured (Phase-1 posture)', () => {
    expect(
      routeBrowserBackend({ url: 'https://news.ycombinator.com/', cloudAvailable: false }),
    ).toBe('local')
  })

  it('matches subdomains of account-sensitive registrable domains', () => {
    expect(isAccountSensitiveUrl('https://m.linkedin.com/feed')).toBe(true)
    expect(isAccountSensitiveUrl('https://linkedin.com.evil.example/feed')).toBe(false)
    expect(isAccountSensitiveUrl('https://notlinkedin.com/feed')).toBe(false)
  })

  it('treats an unparseable URL as not account-sensitive (tool-layer validation rejects it earlier)', () => {
    expect(isAccountSensitiveUrl('not a url')).toBe(false)
  })

  it('honors a workspace-supplied domain list over the default', () => {
    expect(
      routeBrowserBackend({
        url: 'https://app.example-crm.com/inbox',
        cloudAvailable: true,
        accountSensitiveDomains: ['example-crm.com'],
      }),
    ).toBe('local')
    // And the default list still holds its own entries.
    expect(DEFAULT_ACCOUNT_SENSITIVE_DOMAINS).toContain('linkedin.com')
  })
})
