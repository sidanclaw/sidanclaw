...
import type { BrowserCallContext, SessionBundle, SessionVault, VaultSessionInfo } from '../types.js'

function ctx(sessionId: string): BrowserCallContext {
  return { userId: 'user-1', workspaceId: 'ws-1', sessionId }
}

/** In-memory vault standing in for the closed encrypted store (same port). */
function memoryVault(): SessionVault & { bundles: Map<string, SessionBundle & { status: string }> } {
  const bundles = new Map<string, SessionBundle & { status: string }>()
  ...
  return {
    ...
    async put(p) {
      bundles.set(key(p), { ...p.bundle, status: 'active' })
    },
    async markDead(p) {
      const b = bundles.get(key(p))
      if (b) b.status = 'dead'
    ...
    async touch() {},
    async list(p): Promise<VaultSessionInfo[]> {
      return [...bundles.entries()]
        ...
        .map(([k, b]) => ({
          ...
          lastUsedAt: null,
          status: b.status as 'active' | 'dead',
        }))
    },
    async revoke(p) {
...
describe('[COMP:sandbox/session-vault] Session reuse — capture once, no second login (§4.4, §4.8)', () => {
  it('first task hits the login wall, Take-Over captures the session; a LATER task re-injects and lands signed in', async () => {
    ...
    // captures the now-authenticated session into the vault, then the task
    // completes and its sandbox dies.
    await orchestrator.captureSession('task-a', 'github.com')
    await orchestrator.completeTask('task-a')
    expect(vault.bundles.get('user-1:ws-1:github.com')?.status).toBe('active')

    // ── Task 2 (a fresh session, later): the orchestrator injects the
    // vaulted bundle BEFORE the first navigation → no login wall, no second
    // Take-Over.
  ...
  it('silent-death probe: a re-injected session that still login-walls is marked dead (§6)', async () => {
    ...
    expect(vault.bundles.get('user-1:ws-1:github.com')?.status).toBe('dead')
    // A dead bundle is never re-injected on the next task.
    await browser.navigate(ctx('s2'), 'https://github.com/notifications')
    const infos = await vault.list({ userId: 'user-1', workspaceId: 'ws-1' })
