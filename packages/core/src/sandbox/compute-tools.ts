...
export function createComputeTools(opts: CreateComputeToolsOptions): {
  ...
  const runPython = buildTool({
    ...
    async execute(input, context) {
      const gate = autonomousGate(context) ?? workspaceGate(context) ?? (await policyGate('runPython', context))
      if (gate) return gate
  ...
  const loadFromWorkspace = buildTool({
    ...
    timeoutMs: 60_000,
    async execute(input, context) {
      const gate = autonomousGate(context) ?? workspaceGate(context) ?? (await policyGate('loadFromWorkspace', context))
      if (gate) return gate
      if (!opts.files) {
  ...
  const saveToWorkspace = buildTool({
    ...
    timeoutMs: 60_000,
    async execute(input, context) {
      const gate = autonomousGate(context) ?? workspaceGate(context) ?? (await policyGate('saveToWorkspace', context))
      if (gate) return gate
      if (!opts.files) {
        return { data: 'ERROR: Workspace file storage is not configured on this deployment.', isError: true }
      }
