...
#   scripts/publish-windows.sh --exe path/to/sidanclaw.exe
#
...
EXE="$REPO_ROOT/apps/app-desktop/release/sidanclaw.exe"
ALLOW_UNSIGNED=0
FROM_CI=0
EXE_EXPLICIT=0
CI_WORKFLOW="desktop-windows-beta.yml"
CI_ARTIFACT="sidanclaw-windows-unsigned"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      ...
      echo "  --from-ci          download sidanclaw.exe from the latest successful '$CI_WORKFLOW' run, then publish"
      echo "  --allow-unsigned   publish even if the exe's signature can't be verified (a deliberate beta)"
      echo "  --exe <path>       the built installer (default: apps/app-desktop/release/sidanclaw.exe)"
      exit 0 ;;
...
if [[ "$FROM_CI" == "1" ]]; then
  ...
  EXE="$(find "$CI_DIR" -name 'sidanclaw.exe' -type f | head -n1)"
  [[ -n "$EXE" ]] || {
    echo "error: '$CI_ARTIFACT' did not contain sidanclaw.exe." >&2
    exit 1
  }
...
[[ -f "$EXE" ]] || {
  echo "error: $EXE not found. Build it first on Windows: pnpm --filter @sidanclaw/app-desktop package:win" >&2
  exit 1
}
...
EXE_BLOCKMAP="$EXE_DIR/sidanclaw.exe.blockmap"
...
if gh release view "$TAG" --repo "$RELEASES_REPO" >/dev/null 2>&1; then
  ...
  gh release create "$TAG" "${ASSETS[@]}" --repo "$RELEASES_REPO" \
    --title "$TAG" --notes "sidanclaw desktop $TAG (Windows)" --latest
fi
echo "==> Done. The 'Download for Windows' CTA resolves once propagated:"
echo "    https://github.com/$RELEASES_REPO/releases/latest/download/sidanclaw.exe"
